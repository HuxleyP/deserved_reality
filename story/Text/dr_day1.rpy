label dr_day1_intro:
    ...
    menu:
        ...
        "А ты можешь мне карту дать?":
            ...
            ivan "Забыла?"
            sl "Угу."
            "Она ещё немного подумала и пришла к мысли."
    ...
    show sl surprised pioneer normal with dissolve
    "Погружённый в мысли, я на некоторое время впал в ступор, услышав её голос, явно направленный в мою сторону."
    sl "Что-то не так?"
    ivan "А, что? Нет, извини... те."
    show sl normal pioneer with dspr
    sl "Можно и на ты. Ты ведь новенький? Я Славяна, но все зовут меня Славей."
    iv "А сейчас ты скажешь: \"И ты так зови\"."
    sl "И ты так зови." # НАНИ?!
    "Девушка располагала к себе с первых секунд - неплохое достижение для человека с отточенным до автоматизма приветствием, заученность которого можно было раскусить с первых слов."
    ivan "Угу."
    "На момент нависло невловкое молчание."
    sl "А как тебя зовут?"
    ivan "Иван."
    sl "Очень приятно. А можно я тебя буду называть Ваней?"
    ivan "Как хочешь."
    "Славя выполнила свой долг вежливости и получила мой кивок в ответ."
    sl "А почему ты не скажешь, что тебе тоже очень приятно?"
    iv "А мне приятно или не очень?"
    ivan "Зачем?"
    show sl angry pioneer with dspr
    sl "Как зачем? Нужно быть вежливым!"
    #notTODO
...
label sch_day1_camp:
    ...
    ivan "Я? Я Никто."
    "Однако она не на того напала, ибо я совсем не хотел идти с ней на контакт и потенциально был совсем ненинтересной жертвой."
    ...
    if sch_day1_sl_together:
        ...
        "Славе стало смешно от того, как мне стало страшно, и она на секунду рассмеялась, через несколько секунд снова восстановив серьёзную мину."
        sl "Ульяна!"
        $ sch_meet('us', 'Ульяна')
        ...
        us "Ну что-о-о?"
        "Мелкая сморщилась и злобно посмотрела на мою проводницу."
    else:
        show us dontlike pioneer:
            xalign 0.5
            ease 0.5 xalign 0.25
        "Немного оклимавшись, я с удивлением для себя разозлился и собирался стукнуть мелкую по голове, однако она быстро отпрыгнула."
    ...
    "Парень осмотрел нас и отвёл взгляд на Ульяну."
    # Итоговая картина - Славя слева, Ульяна в центре, Шурик справа
